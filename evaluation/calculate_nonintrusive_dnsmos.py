...
def dnsmos_metric(model, audio, fs=16000):
    ...
    Returns:
        dnsmos_score (dict): contains all DNSMOS score including 
    ...
    dnsmos_score = {k: v.item() for k, v in dnsmos_score.items()}
...
def process_one_pair(data_pair, model=None):
    ...
    scores_all = dnsmos_metric(model, inf, fs=fs)
    
    scores = {}
    for metric in METRICS:
            scores[metric] = scores_all[metric]
