...
LastEditors: Ryuk
LastEditTime: 2026-02-22 18:54:56
...
class TestSpectralGainEstimator:
...
if __name__ == "__main__":
    ...
    spectral_gain_estimator_dict = {
        ...
        "WienerSpectralGainEstimator": WienerSpectralGainEstimator
    }

    n_fft = 256
    noisy_audio, sr = librosa.load("./samples/audio_long16noise.wav", sr=None)

    for spectral_gain_estimator_name in spectral_gain_estimator_dict:
        print(f"Testing {spectral_gain_estimator_name}...")
        noise_estimator = MCRANoiseEstimator(n_fft=n_fft)
        spectral_gain_estimator = spectral_gain_estimator_dict[spectral_gain_estimator_name](n_fft=n_fft)
        
        denoiser = TestSpectralGainEstimator(noise_estimator, spectral_gain_estimator, n_fft=n_fft, frame_len=n_fft, hop_len=n_fft//2)
        
        enhanced_audio = denoiser.process(noisy_audio)
        
        sf.write(f"./samples/{'MCRANoiseEstimator'}-{spectral_gain_estimator_name}.wav", enhanced_audio, sr)
